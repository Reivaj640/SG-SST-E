...
function callParentAPI(type, payload) {
    console.log(`[comite-convivencia-viewer.js][callParentAPI] Enviando solicitud al padre. Tipo: ${type}, Payload:`, payload);
    return new Promise((resolve, reject) => {
        ...
        const handleResponse = (event) => {
            ...
            if (response.type === `${type}-response` && response.requestId === requestId) {
                ...
                console.log(`[comite-convivencia-viewer.js][callParentAPI] Respuesta recibida del padre para requestId ${requestId}. Success: ${response.payload && response.payload.success}`);

                if (response.payload && response.payload.success) {
                    resolve(response.payload);
                } else {
                    const errorMessage = (response.payload && response.payload.error) || 'Unknown error from parent process';
                    console.error(`[comite-convivencia-viewer.js][callParentAPI] Error recibido para la solicitud '${type}':`, errorMessage);
...
async function selectDocument(doc) {
    try {
        ...
        console.log(`[comite-convivencia-viewer.js][selectDocument] Documento seleccionado: ${doc.name}, Path: ${doc.path}, Extensión: ${extension}`);
...
async function loadPDF(filePath) {
    console.log(`[comite-convivencia-viewer.js][loadPDF] Solicitando previsualización de PDF para: ${filePath}`);
    try {
...
async function loadExcel(filePath) {
    console.log(`[comite-convivencia-viewer.js][loadExcel] Solicitando previsualización de Excel para: ${filePath}`);
    try {
...
async function loadWord(filePath) {
    console.log(`[comite-convivencia-viewer.js][loadWord] Solicitando previsualización de Word para: ${filePath}`);
    try {
