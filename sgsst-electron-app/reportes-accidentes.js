// reportes-accidentes.js - Componente para la vista de reportes de accidentes
...
class ReportesAccidentesComponent {
    ...
    render() {
        ...
        window.currentReportesAccidentesComponent = this;

        const header = this.createHeader('Ver Reportes de Accidentes', () => this.backToModuleCallback());
        ...
        previewCol.innerHTML = `<div class="preview-placeholder">Seleccione un reporte para previsualizarlo.</div>`;
    ...
    displayItems(items) {
        ...
        if (list.children.length === 0) {
            resultsCol.innerHTML = '<p>No hay reportes o carpetas para mostrar.</p>';
        }
        ...
        if (fileExtension === 'pdf') {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    previewCol.innerHTML = `<div class="preview-error"><h3>Error de Conversión</h3><p>${result.error}</p><button class="btn btn-primary" onclick="window.currentReportesAccidentesComponent.openDocument('${escapedPath}')">Abrir con aplicación externa</button></div>`;
                }
            } catch (error) {
                previewCol.innerHTML = `<div class="preview-error"><h3>Error Inesperado</h3><p>${error.message}</p><button class="btn btn-primary" onclick="window.currentReportesAccidentesComponent.openDocument('${escapedPath}')">Abrir con aplicación externa</button></div>`;
            }
        } else {
            previewCol.innerHTML = `<div class="preview-error"><h3>Previsualización no disponible</h3><p>La previsualización para archivos <strong>.${fileExtension}</strong> no está soportada.</p><button class="btn btn-primary" onclick="window.currentReportesAccidentesComponent.openDocument('${escapedPath}')">Abrir con aplicación externa</button></div>`;
        }
    }
