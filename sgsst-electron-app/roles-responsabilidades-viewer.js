...
function callParentAPI(type, payload) {
    console.log(`[roles-responsabilidades-viewer.js][callParentAPI] Enviando solicitud al padre. Tipo: ${type}, Payload:`, payload);
    return new Promise((resolve, reject) => {
        ...
        const handleResponse = (event) => {
            ...
            if (response.type === `${type}-response` && response.requestId === requestId) {
                ...
                console.log(`[roles-responsabilidades-viewer.js][callParentAPI] Respuesta recibida del padre para requestId ${requestId}. Success: ${response.payload && response.payload.success}`);

                if (response.payload && response.payload.success) {
                    resolve(response.payload);
                } else {
                    const errorMessage = (response.payload && response.payload.error) || 'Unknown error from parent process';
                    console.error(`[roles-responsabilidades-viewer.js][callParentAPI] Error recibido para la solicitud '${type}':`, errorMessage);
...
async function selectDocument(doc) {
    try {
        ...
        console.log(`[roles-responsabilidades-viewer.js][selectDocument] Documento seleccionado: ${doc.name}, Path: ${doc.path}, Extensión: ${extension}`);
...
async function loadPDF(filePath) {
    console.log(`[roles-responsabilidades-viewer.js][loadPDF] Solicitando previsualización de PDF para: ${filePath}`);
    try {
...
async function loadExcel(filePath) {
    console.log(`[roles-responsabilidades-viewer.js][loadExcel] Solicitando previsualización de Excel para: ${filePath}`);
    try {
...
async function loadWord(filePath) {
    console.log(`[roles-responsabilidades-viewer.js][loadWord] Solicitando previsualización de Word para: ${filePath}`);
    try {
