...
- **Permisos por usuario** (desde v0.1.175, 📦702): el acceso a la Bandeja Integrada se gatea con `users.bandeja_integrada_enabled` (default 0). El admin puede condicionar el acceso desde **Configuración > Gestión de Usuario** con el toggle "Acceso a Bandeja Integrada". **Admin global SIEMPRE forzado a `enabled: true`** por el backend (no se puede deshabilitar a sí mismo ni a otros admins). Los tokens de Gmail NO se ven afectados — siguen en `email_connections` por empresa. El gate vive en `renderer.js:1041` (`toggleBandejaIntegrada` → `checkBandejaIntegradaAccess` con fail-open defensivo). El bridge está en `main/bandeja-integrada-permissions-bridge.js` con la firma canónica del proyecto `(app, deps)`.
