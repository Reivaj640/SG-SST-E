  ...
  try {
   ...
   return { success: false, error: { code: 'DELETE_ERROR', message: e.message } };
	...
	ipcMain.handle('mantenimiento:calendario:get-events', async function(_e, params) {
		return getCalendarEventsAll(params);
	});
}
...
/* 📦509 — Devuelve los eventos del calendario para los mantenimientos
   ...
   Task 5 — exportado como fuente de notificaciones. FIX: la extracción
   previa dejó la función DENTRO del register mientras module.exports (a
   nivel de módulo) la referenciaba → ReferenceError al cargar el archivo
   (main.js entero caía). Ahora está a nivel de módulo. Requiere que
   registerMantenimientoHandlers ya se haya llamado (setea _app y
   _getCompanyRootPath). Enriquece cada evento con empresa (los eventos
   base no la traen; notifications-service hace fallback a companies[0]
   sin esto). */
async function getCalendarEventsAll(params) {
	try {
		...
		for (var i = 0; i < allCompanies.length; i++) {
			...
			if (res && res.success && Array.isArray(res.data)) {
				// Task 5 — enriquecer con empresa (los eventos base no la traen;
				// notifications-service hace fallback a companies[0] sin esto)
				for (var k = 0; k < res.data.length; k++) {
					if (res.data[k] && !res.data[k].empresa) res.data[k].empresa = compName;
				}
...
module.exports = { registerMantenimientoHandlers, getCalendarEventsAll };
