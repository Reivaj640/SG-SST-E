...
// ==================== STATUS BADGE ====================
  ...
  function renderChart(meses, valores, meta, proporcionAnual) {
    ...
    // Plugin: zona de fondo meta (verde debajo, rojo encima)
    var metaZonePlugin = {
      id: 'metaZone',
    ...
    // Plugin: línea de proporción anual (violeta)
    var anualLinePlugin = {
      id: 'proporcionAnualLine',
      afterDatasetsDraw: function(chart) {
        if (proporcionAnual <= 0) return;
        var yScale = chart.scales.y;
    ...
    // Plugin: línea de meta (azul) con label
    var metaLinePlugin = {
      id: 'metaLine',
      afterDatasetsDraw: function(chart) {
        if (meta <= 0) return;
    ...
    var allPlugins = [metaZonePlugin, barLabelsPlugin, anualLinePlugin, metaLinePlugin];
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: function(ctx) {
                  ...
                  var estado = val > 0 ? 'EXCEDE' : 'SIN AT MORTAL';
                  return 'Proporción: ' + val.toFixed(2) + '% — ' + estado;
                }
              }
            }
          },
