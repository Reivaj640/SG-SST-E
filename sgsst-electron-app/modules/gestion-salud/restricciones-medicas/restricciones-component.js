// restricciones-component.js - Componente para el submódulo "3.1.6 Restricciones y Recomendaciones Médicas"
...
class RestriccionesMedicasComponent {
    ...
    async handleFilePreviewRequest(event, apiFunctionName) {
        ...
        console.log(`[restricciones-component.js][handleFilePreviewRequest] Solicitud de previsualización recibida. requestId: ${requestId}, filePath: ${filePath}, apiFunctionName: ${apiFunctionName}`);
        try {
            if (!window.electronAPI || typeof window.electronAPI[apiFunctionName] !== 'function') {
                console.error(`[restricciones-component.js][handleFilePreviewRequest] Error: electronAPI.${apiFunctionName} no está disponible.`);
            ...
            console.log(`[restricciones-component.js][handleFilePreviewRequest] Respuesta de electronAPI.${apiFunctionName} para requestId ${requestId}: success=${result.success}, error=${result.error}`);
            ...
            console.error(`[restricciones-component.js][handleFilePreviewRequest] Error al manejar la solicitud de previsualización (${apiFunctionName}) para filePath ${filePath}:`, error);
            event.source.postMessage({
                action: `${apiFunctionName}-response`,
                requestId,
                success: false,
    ...
    async handleFolderRequest(event, apiFunctionName) {
        const { requestId, payload } = event.data;
        console.log(`[restricciones-component.js][handleFolderRequest] Solicitud de carpetas recibida. requestId: ${requestId}, payload:`, payload);
        try {
            if (!window.electronAPI || typeof window.electronAPI[apiFunctionName] !== 'function') {
                console.error(`[restricciones-component.js][handleFolderRequest] Error: electronAPI.${apiFunctionName} no está disponible.`);
                throw new Error(`electronAPI.${apiFunctionName} no está disponible.`);
            }
            const result = await window.electronAPI[apiFunctionName](payload);
            console.log(`[restricciones-component.js][handleFolderRequest] Respuesta de electronAPI.${apiFunctionName} para requestId ${requestId}: success=${result.success}`);
            event.source.postMessage({
                action: `${apiFunctionName}-response`,
                requestId,
                success: result.success,
            ...
            console.error(`[restricciones-component.js][handleFolderRequest] Error al manejar la solicitud de carpetas:`, error);
            event.source.postMessage({
                action: `${apiFunctionName}-response`,
                requestId,
                success: false,
    ...
    async handleDownloadRequest(event, apiFunctionName) {
        ...
        console.log(`[restricciones-component.js][handleDownloadRequest] Solicitud de descarga recibida. requestId: ${requestId}, filePath: ${filePath}`);
        try {
            if (!window.electronAPI || typeof window.electronAPI[apiFunctionName] !== 'function') {
                console.error(`[restricciones-component.js][handleDownloadRequest] Error: electronAPI.${apiFunctionName} no está disponible.`);
                throw new Error(`electronAPI.${apiFunctionName} no está disponible.`);
            }
            const result = await window.electronAPI[apiFunctionName](filePath);
            console.log(`[restricciones-component.js][handleDownloadRequest] Respuesta de electronAPI.${apiFunctionName} para requestId ${requestId}: success=${result.success}`);
            event.source.postMessage({
                action: `${apiFunctionName}-response`,
                requestId,
                success: result.success,
            ...
            console.error(`[restricciones-component.js][handleDownloadRequest] Error al manejar la solicitud de descarga:`, error);
            event.source.postMessage({
                action: `${apiFunctionName}-response`,
                requestId,
                success: false,
    ...
    async handleOpenPathRequest(event, apiFunctionName) {
        ...
        console.log(`[restricciones-component.js][handleOpenPathRequest] Solicitud de apertura de ruta recibida. requestId: ${requestId}, path: ${pathToOpen}`);
        try {
            if (!window.electronAPI || typeof window.electronAPI[apiFunctionName] !== 'function') {
                console.error(`[restricciones-component.js][handleOpenPathRequest] Error: electronAPI.${apiFunctionName} no está disponible.`);
                throw new Error(`electronAPI.${apiFunctionName} no está disponible.`);
            }
            const result = await window.electronAPI[apiFunctionName](pathToOpen);
            console.log(`[restricciones-component.js][handleOpenPathRequest] Respuesta de electronAPI.${apiFunctionName} para requestId ${requestId}: success=${result.success}`);
            event.source.postMessage({
                action: `${apiFunctionName}-response`,
                requestId,
                success: result.success,
