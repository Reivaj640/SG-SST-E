...
class RendicionCuentasComponent {
   ...
   */
  async render() {
    // Cargar el viewer que contiene la interfaz
    // En el contexto del navegador, no usamos require, sino que directamente llamamos al viewer
    if (typeof window !== 'undefined' && window.rendicionViewer) {
      await window.rendicionViewer.render(this.container, {
        company: this.company,
        module: this.module,
        submodule: this.submodule
      });
    } else {
      console.error('El viewer del submódulo rendicion-cuentas no está disponible');
    }
  }
