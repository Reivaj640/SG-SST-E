...
class PoliticaComponent {
        ...
        if (event.data.type.endsWith('-request')) {
            ...
            switch (action) {
                ...
                case 'download-document':
                    ...
                    break;
                default:
                    console.warn(`[PoliticaComponent] Acción no manejada: ${action}`);
            }
        }
    }
    ...
    async handleStandardRequest(event, apiFunctionName) {
        ...
        console.log(`[PoliticaComponent] Solicitud: ${apiFunctionName}, ID: ${requestId}`);
        ...
        try {
            ...
            console.error(`[PoliticaComponent] Error en ${apiFunctionName}:`, error);
            event.source.postMessage({
                type: `${event.data.type.replace('-request', '')}-response`,
                requestId,
                payload: {
