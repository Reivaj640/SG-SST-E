/* ═══════════════════════════════════════════════════════════════════
   K+AIR · Módulo 7.1.3 — Acciones de Mejora — AT y EL
   Viewer Logic
   ═══════════════════════════════════════════════════════════════════ */

const SUBMODULO = '7.1.3';
const COMPANY = new URLSearchParams(window.location.search).get('company') || 'Constructora ABC S.A.S';
const ORIGEN_OPTIONS = {
  'investigacion-at': 'Investigación de AT',
  'investigacion-el': 'Investigación de EL',
  'analisis-incidente': 'Análisis de Incidente'
};

const state = { editingId: null, deleteTarget: null, data: [] };

function syncStore() {
  if (window.MejoramientoStore) window.MejoramientoStore.update('713', state.data);
}
...
function renderTable() {
  ...
  if (rows.length === 0) {
    tbody.innerHTML = `<tr><td colspan="10"><div class="kair-empty"><i class="fas fa-clipboard-list"></i><div class="kair-empty__title">No se encontraron acciones</div><div class="kair-empty__desc">No hay acciones que coincidan con los filtros.</div></div></td></tr>`;
  } else {
