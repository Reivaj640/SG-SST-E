// acciones-pc-logic.js - Componente para el submódulo "7.1.1 Acciones Preventivas y Correctivas"

class AccionesPcComponent {
        ...
        if (event.data.type.endsWith('-request')) {
            ...
            switch (action) {
                ...
                case 'delete-document':
                    ...
                    break;
                default:
                    console.warn(`[AccionesPcLogic] Acción no manejada: ${action}`);
            }
        }
    }
    ...
    async handleStandardRequest(event, apiFunctionName) {
        ...
        console.log(`[AccionesPcLogic] Solicitud: ${apiFunctionName}, ID: ${requestId}`);
        ...
        try {
            ...
            console.error(`[AccionesPcLogic] Error en ${apiFunctionName}:`, error);
            event.source.postMessage({
                type: `${event.data.type.replace('-request', '')}-response`,
                requestId,
                payload: {
...
window.AccionesPcComponent = AccionesPcComponent;
