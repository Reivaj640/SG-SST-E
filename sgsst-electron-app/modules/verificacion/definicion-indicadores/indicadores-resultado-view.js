/* ═══════════════════════════════════════════════════════════════════
   K+AIR · Submódulo 6.1.1 — Vista: Indicadores de RESULTADO
   ═══════════════════════════════════════════════════════════════════ */

var IndicadoresResultadoView = (function () {
  'use strict';
  ...
  function _getFiltered() {
    var items = IndicadoresService.getIndicadoresByType('RESULTADO');
    ...
    if (items.length === 0) {
      _container.innerHTML =
        '<div class="kair-ind-empty">' +
          ...
          '<p class="kair-ind-empty__desc">' + (_searchQuery ? 'Intenta ajustar los términos de búsqueda' : 'No hay indicadores de resultado definidos') + '</p>' +
        '</div>';
      return;
    }
      ...
      '<div class="kair-ind-table-wrapper">' +
        '<table class="kair-ind-table">' +
          '<thead><tr>' +
            ...
            '<th onclick="IndicadoresResultadoView._toggleSort(\'name\')" style="cursor:pointer">Indicador ' + _renderSortIcon('name') + '</th>' +
            '<th>Frecuencia</th>' +
            '<th onclick="IndicadoresResultadoView._toggleSort(\'target\')" style="cursor:pointer">Meta ' + _renderSortIcon('target') + '</th>' +
            '<th>Resultado Actual</th>' +
            '<th onclick="IndicadoresResultadoView._toggleSort(\'status\')" style="cursor:pointer">Estado ' + _renderSortIcon('status') + '</th>' +
