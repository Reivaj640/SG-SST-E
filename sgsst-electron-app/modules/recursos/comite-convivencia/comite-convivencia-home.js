// comite-convivencia-home.js - Lógica del Portal Comité de Convivencia
// Este script se carga dinámicamente desde comite-convivencia-portal-logic.js

// NO usar DOMContentLoaded porque el HTML se carga vía fetch

function initializePortal() {
  console.log('[cc-home] initializePortal() INICIADO');
}

function goBackToModule() {
  if (window.comiteConvivenciaPortalComponent) {
    window.comiteConvivenciaPortalComponent.destroy();
  }
...
function enterRepositorioActas() {
  if (window.comiteConvivenciaPortalComponent) {
    window.comiteConvivenciaPortalComponent.enterViewer();
  } else {
...
function realizarNuevaActa() {
  if (window.comiteConvivenciaPortalComponent) {
    window.comiteConvivenciaPortalComponent.showRealizarActas();
  } else {
    window.KAIRToast && window.KAIRToast.show('Error: No se pudo acceder al generador de actas.', 'error');
  }
}

function openProtocoloPrevencion() {
  window.KAIRToast && window.KAIRToast.show('Protocolo de Prevención', 'info', { subtitle: 'Próximamente' });
}

function openMarcoNormativo() {
  window.KAIRToast && window.KAIRToast.show('Marco Normativo CCL', 'info', { subtitle: 'Próximamente' });
}
