// curso-virtual-logic.js - Componente para el submódulo "1.2.3 Curso Virtual 50 Horas"
...
class CursoVirtualComponent {
    ...
    render() {
        ...
        const viewerUrl = `modules/recursos/curso-virtual/curso-virtual-view.html?company=${encodeURIComponent(this.companyName)}&module=${encodeURIComponent(this.moduleName)}&submodule=${encodeURIComponent(this.submoduleName)}`;
    ...
    handleIframeMessage(event) {
        ...
        // 📦608-fix15 — El iframe (curso-virtual-viewer.js) nos pide abrir el
        ...
        if (event.data.type === 'open-file-viewer-modal') {
            ...
            if (window.kairFV && typeof window.kairFV.openWithFileViewerFromPath === 'function') {
                ...
                console.warn('[CursoVirtualLogic] kairFV.openWithFileViewerFromPath no disponible');
            }
            return;
        }
        ...
        if (event.data.type.endsWith('-request')) {
            ...
            switch (action) {
                ...
                case 'download-document':
                    ...
                    break;
                default:
                    console.warn(`[CursoVirtualLogic] Acción no manejada: ${action}`);
            }
        }
    }
