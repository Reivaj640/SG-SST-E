// plan-prevencion-logic.js - Componente para el submódulo "5.1.1 Plan de Prevención de Emergencias"

class PlanPrevencionComponent {
    ...
    render() {
        ...
        const viewerUrl = `modules/gestion-amenazas/plan-prevencion/plan-prevencion-view.html?company=${encodeURIComponent(this.companyName)}&module=${encodeURIComponent(this.moduleName)}&submodule=${encodeURIComponent(this.submoduleName)}`;
    ...
    handleIframeMessage(event) {
        ...
        if (event.data.type === 'open-file-viewer-modal' && event.data.filePath) {
            if (window.kairFV && typeof window.kairFV.openWithFileViewerFromPath === 'function') {
                ...
                console.warn('[PlanPrevencionLogic] kairFV.openWithFileViewerFromPath no disponible');
            }
            return;
        }
        ...
        if (event.data.type.endsWith('-request')) {
            ...
            switch (action) {
                ...
                case 'delete-document':
                    ...
                    break;
                default:
                    console.warn(`[PlanPrevencionLogic] Acción no manejada: ${action}`);
            }
        }
    }
    ...
    async handleStandardRequest(event, apiFunctionName) {
        ...
        console.log(`[PlanPrevencionLogic] Solicitud: ${apiFunctionName}, ID: ${requestId}`);
        ...
        try {
            ...
            console.error(`[PlanPrevencionLogic] Error en ${apiFunctionName}:`, error);
            event.source.postMessage({
                type: `${event.data.type.replace('-request', '')}-response`,
                requestId,
                payload: {
...
window.PlanPrevencionComponent = PlanPrevencionComponent;
