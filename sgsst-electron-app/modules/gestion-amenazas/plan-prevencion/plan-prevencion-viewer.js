...
function callParentAPI(type, payload) {
    console.log(`[plan-prevencion-viewer.js][callParentAPI] Enviando solicitud al padre. Tipo: ${type}, Payload:`, payload);
    return new Promise((resolve, reject) => {
        ...
        const handleResponse = (event) => {
            ...
            if (response.type === `${type}-response` && response.requestId === requestId) {
                ...
                console.log(`[plan-prevencion-viewer.js][callParentAPI] Respuesta recibida del padre para requestId ${requestId}. Success: ${response.payload && response.payload.success}`);

                if (response.payload && response.payload.success) {
                    resolve(response.payload);
                } else {
                    const errorMessage = (response.payload && response.payload.error) || 'Unknown error from parent process';
                    console.error(`[plan-prevencion-viewer.js][callParentAPI] Error recibido para la solicitud '${type}':`, errorMessage);
...
async function selectDocument(doc) {
    try {
        ...
        console.log(`[plan-prevencion-viewer.js][selectDocument] Documento seleccionado: ${doc.name}, Extensión: ${extension}`);
