/* ═══════════════════════════════════════════════════════════════════
   K+AIR · 5.1.2 Exámenes Médicos Brigadista · Visualizador v2.0
   ...
   - Logging estructurado [K+AIRSST][EXAMENES_BRIGADISTA][ACCION][STATUS]

   MIGRACIÓN → v2.0:
   - Namespace legacy (sidebar, app-header) → kair-* (canónico)
   - Removida dependencia font-awesome (reemplazada por bootstrap-icons)
   - Removida dependencia Google Fonts (usa --kair-font del sistema)
   ...
   - Handlers en renderer.js y examenes-brigadista-logic.js SIN MODIFICAR
   - main.js, preload.js SIN MODIFICAR

   ═══════════════════════════════════════════════════════════════════ */

var ExamenesBrigadistaViewer = (function () {
    'use strict';
    ...
    /* ═══════════════════════════════════════════════════════════════
       CONSTANTES
       ═══════════════════════════════════════════════════════════════ */
    var _LOG_PREFIX = '[K+AIRSST][EXAMENES_BRIGADISTA]';
    ...
    function _renderPreview(result, filePath) {
        ...
        if (!container) return;
        _hideLoading(); // 📦608-fix16: ocultar loading en TODAS las ramas
        container.innerHTML = '';
    ...
    function _expandFileViewer(filePath) {
        if (!filePath) return;
        try {
            window.top.postMessage({ type: 'open-file-viewer-modal', filePath: filePath, source: 'examenes-brigadista' }, '*');
        } catch (e) {
    ...
    window.ExamenesBrigadistaViewerGoUp = _goUpLevel;
    window.ExamenesBrigadistaViewerResetToRoot = _navigateToRoot;
    window.ExamenesBrigadistaViewerShowToast = _showToast;
    window.ExamenesBrigadistaViewerShowConfirm = _showConfirmModal;
    window.ExamenesBrigadistaViewerHideConfirm = _hideConfirmModal;
...
window.ExamenesBrigadistaViewer = ExamenesBrigadistaViewer;

/* ═══════════════════════════════════════════════════════════════════
   AUTO-INIT AL CARGAR EL DOM
   ═══════════════════════════════════════════════════════════════════ */
document.addEventListener('DOMContentLoaded', function () {
    ExamenesBrigadistaViewer.init();
});
