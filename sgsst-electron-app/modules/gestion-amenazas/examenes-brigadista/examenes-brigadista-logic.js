// examenes-brigadista-logic.js - Componente para el submódulo "5.1.2 Exámenes Médicos Brigadista"

class ExamenesBrigadistaComponent {
    ...
    render() {
        ...
        const viewerUrl = `modules/gestion-amenazas/examenes-brigadista/examenes-brigadista-view.html?company=${encodeURIComponent(this.companyName)}&module=${encodeURIComponent(this.moduleName)}&submodule=${encodeURIComponent(this.submoduleName)}`;
    ...
    handleIframeMessage(event) {
        ...
        if (event.data.type === 'open-file-viewer-modal' && event.data.filePath) {
            if (window.kairFV && typeof window.kairFV.openWithFileViewerFromPath === 'function') {
                ...
                console.warn('[ExamenesBrigadistaLogic] kairFV.openWithFileViewerFromPath no disponible');
            }
            return;
        }
        ...
        if (event.data.type.endsWith('-request')) {
            ...
            switch (action) {
                ...
                case 'delete-document':
                    ...
                    break;
                default:
                    console.warn(`[ExamenesBrigadistaLogic] Acción no manejada: ${action}`);
            }
        }
    }
    ...
    async handleStandardRequest(event, apiFunctionName) {
        ...
        console.log(`[ExamenesBrigadistaLogic] Solicitud: ${apiFunctionName}, ID: ${requestId}`);
        ...
        try {
            ...
            console.error(`[ExamenesBrigadistaLogic] Error en ${apiFunctionName}:`, error);
            event.source.postMessage({
                type: `${event.data.type.replace('-request', '')}-response`,
                requestId,
                payload: {
...
window.ExamenesBrigadistaComponent = ExamenesBrigadistaComponent;
