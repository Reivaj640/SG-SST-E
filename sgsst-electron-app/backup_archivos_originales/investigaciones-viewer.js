...
// Cargar contenido de una carpeta (carpetas y documentos)
async function loadFolderContents(folderPath) {
    try {
        ...
        if (result.success) {
            console.log('VIEWER: La carga de carpetas y archivos fue exitosa. Renderizando...');
            renderFolders(result.folders);
            renderDocuments(result.files);
        } else {
            showNotification(`Error al cargar contenido: ${result.error}`, 'error');
        }
    } catch (error) {
